#!/usr/bin/env python3
"""
가상 모터 드라이버 - 방식 2: 각속도(w) 우선 클리핑

특징:
  - 경로 추종에 필수인 w(각속도)를 최우선 보장
  - w 실행 후 남은 자원으로만 v(선속도) 허용
  - Pure Pursuit 자율주행에 최적화
장점:
  - 경로 이탈 방지 (w가 보장되어 정확한 곡률 유지)
  - 급커브에서 자동 감속 (안전성 향상)
동작:
  - 급커브: w에 자원 집중 → v 자동 감속 또는 정지
  - 직선: w=0 → v 최대 활용
"""
...
class MotorCmdVelSim2(Node):
    ...
    def cmd_vel_callback(self, msg: Twist):
        """
        Angular Priority Clipping
        1. angular vel clipping
        2. linear vel clipping based on remaining capacity
        3. final verification and publish
        """
        v_cmd = msg.linear.x
        w_cmd = msg.angular.z
