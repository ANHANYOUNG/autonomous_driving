...
class StateMachineExecutor(Node):
    ...
    def __init__(self):
        ...
        # 6. 주기적인 동작 수행용 타이머 (예: STOP 상태에서 계속 정지 명령)
        self.publish_timer = self.create_timer(0.1, self.loop)
    ...
    def state_callback(self, msg):
        ...
        if old_state == 'RUN':
            self.ppc_enable_pub.publish(Bool(data=False))
            self.get_logger().info('Exiting RUN: PPC disabled.')
        ...
        #     self.selected_path_name = None
        #     self.selected_path_points = None
        #     self.get_logger().info('Entered RUN mode. Waiting for path selection.')
        elif new_state == 'RUN':
            self.get_logger().info('Entered RUN mode.')
            self.ppc_enable_pub.publish(Bool(data=True))

        elif new_state == 'KEY':
            self.get_logger().info('Entered KEY mode.')
    ...
    def loop(self):
        ...
        if self.current_state == 'STOP':
            # STOP 상태에서는 계속 정지 명령을 보냄
            self.stop_pub.publish(self.zero_twist)

    # path select + AUTO_START order + ppc_path publishing
    # def command_callback(self, msg):
    #     """
    #     /state_command를 처리합니다.
    #     '실행기'는 RUN 상태일 때의 세부 명령('path_A' 등)만 관심 있습니다.
    #     """
    #     if self.current_state != 'RUN':
    #         return # RUN 상태가 아니면 세부 명령 무시
    ...
    #             self.ppc_enable_pub.publish(Bool(data=True))
    #             self.get_logger().info(f'RUN started. Path {self.selected_path_name} published. PPC enabled.')
